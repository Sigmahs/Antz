...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            place.ant = self
        elif place.ant is not None:
            if self.can_contain(place.ant):
                assert place.ant.can_contain(self) == False, 'Two ants in {0}'.format(place)
                self.contain_ant(place.ant)
                place.ant = self
            elif place.ant.can_contain(self):
                assert place.ant.contained_ant == None, 'Two ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        radius, keepspot = 0, self.place
        while keepspot:
            if keepspot != beehive:  
                if random_or_none(keepspot.bees):
                    if radius >= self.min_range:
                        if radius <= self.max_range:
                            return random_or_none(keepspot.bees)
                        else:
                            keepspot = keepspot.entrance
                            radius += 1
                    else:
                        keepspot = keepspot.entrance
                        radius += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        beeshere = range(len(list(self.place.bees)))
        originalbees = list(self.place.bees)
        if amount >= self.armor:
            for count in beeshere:
                originalbees[count].reduce_armor(self.damage + amount)
        else:
            for count in beeshere:
                originalbees[count].reduce_armor(amount)
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        originalbees = list(self.place.bees)
        for count in beeshere:
                originalbees[count].reduce_armor(self.damage) 
        # END Problem 7
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        def antbuffer(ant):
            if ant.buffed == False:
                queenspot.ant.damage *= 2
                queenspot.ant.buffed = True
            if ant.contained_ant:
                if ant.contained_ant.buffed == False:
                    queenspot.ant.contained_ant.damage *= 2
                    queenspot.ant.contained_ant.buffed = True

        if self.is_impostor == False:
            queenspot = self.place       
            while queenspot.exit != None:
                queenspot = queenspot.exit
                if queenspot.ant != None:
                    antbuffer(queenspot.ant)  
    ...
    def remove_from(self, place):
        # Special handling for container ants   
        if place.ant.is_impostor == False and self.place.bees:
            bees_win()
        if place.ant.is_impostor == False:
            return None
        elif place.ant.contained_ant and place.ant.contained_ant.is_impostor == False:
            return None
        else:
            Ant.remove_from(self, place)
...
class Bee(Insect):
    ...
    def action(self, gamestate, fear = False):
...
def make_slow(action, bee):
    ...
    """
    # BEGIN Problem EC
    def slowed(gamestate, fear = False):
        if gamestate.time % 2 == 1:
            return None
        else:
            return action(gamestate, fear)
    return slowed
    # END Problem EC

def make_scared(action, bee):
    return bee.action
...
def apply_effect(effect, bee, duration):
    ...
    infected_action = effect(bee.action, bee)

    def count_duration(gamestate, fear = False):
        nonlocal duration
        nonlocal bee

        if duration > 0:
            duration -= 1
            return infected_action(gamestate, fear)

        elif duration < 0:
            duration += 1
            fear = True

        return normal_action(gamestate, fear)

    bee.action = count_duration
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target and not target in ScaryThrower.scared_bees:
            apply_effect(make_scared, target, -2)
